  ...
  },
  {
    ...
    cell: ({ row }: any) => (
      <div className="flex flex-col gap-0.5">
        ...
        <div className="text-xs text-gray break-all">{row.original.ownerAddress}</div>
      </div>
    ),
  },
]
...
const AllUsernamesTable = ({
  ...
  return (
    <div className="flex flex-col gap-3">
      <Heading variant="h3" title={CONSTANTS.TABLE.USERNAMES_HEADING} color="white" fontWeight={700} />
      ...
      <div className="rounded-md border border-header bg-header/40 p-4 text-secondary">
        ...
        {error && (
          <div className="flex flex-col gap-2">
            <p>We ran into a problem fetching usernames.</p>
          ...
          <div className="flex flex-col gap-3">
            {isRefetching && (
              <div className="flex items-center gap-2 rounded border border-header/50 bg-header/30 p-2 text-sm text-gray">
                <Spinner size="sm" />
                <span>Refreshing usernames…</span>
              </div>
            )}
